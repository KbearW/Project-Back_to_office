...
select models.name as model_name, brands.name as brand_name, brands.headquarters as headquarters from models left join brands on models.brand_id = brands.brand_id where models.name = 'Mustang' and brands.name = 'Ford';
...
SELECT models.year, models.name, models.model_id, a.name FROM models full join awards as a on models.model_id = a.winner_id WHERE models.year >= 1964;
...
SELECT b.name, m.name, m.year, b.discontinued, b.discontinued-m.year as years_unitl_brand_discontinued FROM models as m LEFT JOIN brands AS b on m.brand_id = b.brand_id WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT brand_id FROM models GROUP BY 1 HAVING count(brand_id)>5 ORDER by count(brand_id) DESC;
